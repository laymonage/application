...
router.post('/company/', function(req, res){
    ...
    if (!validator.isAlphanumeric(country_code)){
      req.session.flash_error('Negara harus hanya mengandung huruf dan angka');
    }
    ...
    .then(function(company){
      ...
      if ( _.indexOf( company.get_available_date_formats(), date_format ) < 0 ) {
        var error_msg = 'Format tanggal tidak diketahui';
    ...
    .then(function(){
        req.session.flash_message('Kantor berhasil diperbarui');
        return res.redirect_with_session('/settings/general/');
    })
    .catch(function(error){
        ...
        req.session.flash_error(
            'Gagal memperbarui detail kantor, silakan kontak layanan pelanggan'
        );
  ...
  req.user.getCompany()
    ...
    .then(function(){
      req.session.flash_message( schedule.is_user_specific()
        ? 'Jadwal untuk karyawan telah tersimpan'
        : 'Jadwal untuk kantor telah tersimpan'
      );
    })
    ...
    .catch(function(error){
      ...
      req.session.flash_error( schedule.is_user_specific()
        ? 'Gagal menyimpan jadwal karyawan'
        : 'Gagal menyimpan jadwal kantor'
      );
...
router.post('/bankholidays/', function(req,res){
    ...
    .catch(function(error){
        ...
        req.session.flash_error(
            'Gagal memperbarui detail hari libur nasional, silakan hubungi layanan pelanggan'
        );
...
router.post('/bankholidays/import/', (req, res) => {
  ...
  Promise
    ...
    .then( created_bank_holidays => {
      ...
      if ( created_bank_holidays.length && created_bank_holidays.length > 0) {
        req.session.flash_message(
          'Hari libur nasional baru telah ditambahkan: '
          + created_bank_holidays.map(bh => bh.name).join(', ')
        );
      } else {

        req.session.flash_message('Tak ada lagi hari libur nasional');
      }
    ...
    .catch(error => {
      ...
      req.session.flash_error(
        'Gagal mengimpor hari libur nasional'
      );
...
router.post('/bankholidays/delete/:bank_holiday_number/', function(req, res){
    ...
    if (!validator.isInt(bank_holiday_number)) {
        ...
        req.session.flash_error('Gagal menghapus hari libur nasional: parameter salah');
    ...
    .then(function(){
        req.session.flash_message('Hari libur nasional berhasil dihapus');
        return res.redirect_with_session('/settings/general/');
    });
});
...
router.post('/leavetypes', function(req, res){
  ...
  req.user
    ...
    .then(() => {
      req.session.flash_message('Perubahan pada tipe cuti berhasil disimpan');
      return res.redirect_with_session('/settings/general/');
    })
    .catch(error => {
      console.error(
      ...
      req.session.flash_error(
        'Gagal memperbarui detail tipe cuti, silakan hubungi layanan pelanggan'
      );
...
router.post('/leavetypes/delete/:leave_type_id/', function(req, res){
    ...
    if (!validator.isInt(leave_type_id)) {
        ...
        req.session.flash_error('Gagal menghapus tipe cuti: parameter salah');
    ...
    .then(function(company){
        ...
        // Check if there exist leaves for current type and if so, do not remove it
            ...
            req.session.flash_error('Gagal menghapus tipe cuti: tipe sedang digunakan');
    ...
    .then(function(){
        req.session.flash_message('Tipe cuti berhasil dihapus');
        return res.redirect_with_session('/settings/general/');
    })
    .catch(function(error){
        console.error(
...
router.post('/company/authentication/', function(req, res){
  ...
  req.user
    ...
    .catch(function(error){
      ...
      req.session.flash_error(
        'Gagal memperbarui konfigurasi LDAP. ' +
        ( error.show_to_user ? error : 'Silakan kontak layanan pelanggan')
      );
...
function get_and_validate_bank_holiday(args) {
  ...
  if ( ! validator.isDate(date) ) {
    req.session.flash_error(
      'Hari baru untuk '+item_name+' harus berupa tanggal'
    );
  }
...
function get_and_validate_leave_type(args) {
  ...
  if ( ! validator.matches(color, /^leave_type_color_\d+$/)) {
    throw_user_error( 'Warna baru untuk '+item_name+' harus berupa class css yang valid' );
  }

  if ( ! validator.isNumeric(limit) ){
    throw_user_error( 'Batasan baru untuk '+item_name+' harus numerik' );

  } else if ( limit < 0) {
    throw_user_error( 'Batasan baru untuk '+item_name+' harus berupa angka positif atau 0' );
  }
...
function get_and_validate_ldap_auth_configuration(args) {
  ...
  if (!validator.matches(url, /^ldaps?:\/\/[a-z0-9\.\-]+:\d+$/i)){
    req.session.flash_error(
      "URL ke peladen LDAP harus dalam format: 'ldap://HOSTNAME:PORT'"
    );
  }
...
router.get('/company/backup/', (req, res) => {
  ...
  req.user
    ...
    .catch(function(error){
      ...
      req.session.flash_error(
        "Gagal mengunduh ringkasan kantor. " +
        ( error.show_to_user ? error : 'Silakan kontak layanan pelanggan')
      );
...
router.post('/company/delete/', (req, res) => {
  ...
  req.user
    ...
    .then(() => {
      req.session.flash_message(`Kantor ${company.name} dan data terkait berhasil dihapus`);
    ...
    .catch(error => {
      ...
      req.session.flash_error(
        `Gagal menghapus kantor. Alasan: ${ Exception.extract_user_error_message(error) }`
      );
