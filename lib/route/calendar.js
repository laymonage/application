...
router.post('/bookleave/', function(req, res){
      ...
      (users, company, valide_attributes) => {
        ...
        if (company.is_mode_readonly_holidays() ){
          req.session.flash_error(
            "Akun kantor terkunci dan permohonan "
            + "baru tidak dapat ditambahkan. Silakan hubungi administrator."
          );
          throw new Error('Kantor dalam mode "Libur baca-saja"');
        }
    ...
    .then(function(){
        ...
        req.session.flash_message('Permohonan cuti baru telah ditambahkan');
    ...
    .catch(function(error){
        ...
        req.session.flash_error('Gagal membuat permohonan cuti');
...
router.get('/teamview/', function(req, res){
  ...
  Promise.join(
    ...
    .catch(error => {
      ...
      req.session.flash_error('Gagal mengakses laman Tinjauan Tim. Silakan hubungi administrator.');
      if (error.hasOwnProperty('user_message')) {
        req.session.flash_error(error.user_message);
      }
      return res.redirect_with_session('/');
...
router.post('/feeds/regenerate/', function(req, res){
  ...
  req.user
    ...
    .then(function(){
        req.session.flash_message('Umpan telah diperbarui');
