...
router.post('/add/', function(req, res){
  ...
  .then(function(){
    if ( req.session.flash_has_errors() ) {
      ...
      req.session.flash_message('Karyawan baru berhasil ditambahkan');
  ...
  parseAsync
    ...
    .then(args => {
      ...
      if (files.users_import.size === 0) {
        Exception.throw_user_error({
          user_error   : 'Tidak ada berkas .CSV yang disediakan untuk dipulihkan',
          system_error : 'Pengguna ' + req.user.id + ' mencoba untuk mengimpor karyawan '
            + 'tanpa memberikan berkas .CSV',
        });
      } else if ( files.users_import.size > 2097152 ) {
        Exception.throw_user_error({
          user_error   : 'Berkas .CSV tidak dapat lebih besar dari 2M',
          system_error : 'Pengguna ' + req.user.id + ' mencoba untuk menggunakan berkas '
            + 'lebih besar dari 2M',
        });
      }
    ...
    .then(parsed_data => {
      ...
      if (parsed_data.length > 201) {
        Exception.throw_user_error({
          user_error : 'Tidak dapat mengimpor lebih dari 200 karyawan dalam sekali jalan. '
            + 'Silakan pecah berkas .CSV menjadi bagian dengan kurang dari 200 karyawan '
            + 'dan proses satu per satu',
          system_error : 'Pengguna ' + req.user.id + ' mencoba mengimpor lebih dari 200 '
            + 'karyawan sekaligus'
        });
      }
    ...
    .then(action_result => {
      ...
      if ( action_result.users.length > 0 ) {
        req.session.flash_message(
          'Sukses mengimpor pengguna dengan surel berikut: '
      ...
      if (action_result.errors.length > 0) {
        action_result.errors.forEach(err => req.session.flash_error(
          'Gagal menambahkan karyawan dengan surel ' + err.email + '. Alasan: ' + err.error
        ));
      }
    ...
    .catch(function(error){
      ...
      req.session.flash_error(
        'Gagal mengimpor karyawan, alasan: '
    ...
    return model.User.find_by_email(new_user_attributes.email)
      .then(function(user){
        ...
        if (user && user.companyId !== employee.companyId) {
          var error_msg = 'Terdapat akun aktif dengan surel yang sama di sistem.';
...
var ensure_email_is_not_used_elsewhere = function(args){
  ...
  return model.User
    ...
    .then(function(user){
      ...
      if (user) {
        req.session.flash_error('Surel sudah pernah digunakan');
        throw new Error('Surel sudah pernah digunakan');
      }
  ...
  if (
    ...
    return model.User
      ...
      .then(function(number_of_admins_to_be_left){
        ...
        req.session.flash_error('Ini adalah admin terakhir di kantor. Tidak dapat mencabut hak admin.');
        throw new Error('Percobaan untuk mencabut hak admin terakhir di kantor '+employee.companyId);
      });
  }
...
router.post('/edit/:user_id/', function(req, res){
  ...
  .then(function(){
    ...
    employee
      ...
      .then(function(){
        req.session.flash_message(
          'Detail untuk '+employee.full_name()+' telah diperbarui'
        );
...
router.post('/delete/:user_id/', function(req, res){
    ...
    .then(function(result){
        req.session.flash_message(
            'Data karyawan telah dihapus dari sistem'
        );
        return res.redirect_with_session('../..');
    })
    .catch(function(error){
...
router.all('/search/', function(req, res){
  ...
  if ( ! validator.isEmail( email )) {
    req.session.flash_error('Surel yang diberikan tidak terlihat valid: "'+email+'"');
    return res.json([]);
  }
...
function get_and_validate_user_parameters(args) {
  ...
  console.log('>>>>>: ' + email);
        req.session.flash_error(
            'Surel baru untuk '+item_name+' harus merupakan alamat surel yang valid'
        );
    }

    if (!validator.isNumeric(department_id)) {
        req.session.flash_error(
            'Nomor departemen baru untuk '+item_name+' harus merupakan nomor yang valid'
        );
    }

    if (adjustment && ! validator.isFloat(adjustment) ) {
      req.session.flash_error(
        'Penyesuaian jatah cuti yang baru untuk '+item_name+' harus berupa bilangan yang valid'
      );
    } else if (adjustment && ! ( adjustment % 1 === 0 || Math.abs( adjustment % 1 ) === 0.5 )) {
      req.session.flash_error(
        'Penyesuaian jatah cuti yang baru untuk '+item_name+' harus berupa bilangan bulat atau dengan setengah'
      );
    }
    ...
    if (!validator.isDate(start_date)) {
      req.session.flash_error(
        'Tanggal mulai baru untuk '+item_name+' harus merupakan tanggal yang valid'
      );
    }
    ...
    if (end_date ){
      ...
      if ( ! validator.isDate(end_date)) {
        req.session.flash_error(
          'Tanggal akhir baru untuk '+item_name+' harus merupakan tanggal yang valid'
        );
      }
    }
    ...
    if (
        ...
        req.session.flash_error(
            'Tanggal akhir untuk '+item_name+' sebelum tanggal mulai'
        );
    }
    ...
    if (require_password && ! password) {
      req.session.flash_error('Kata sandi diperlukan');
    }
...
function ensure_user_id_is_integer(args){
    ...
    if (! validator.isInt(user_id)){
        throw new Error(
          'Pengguna '+req.user.id+' mencoba mengubah ID dengan nilai selain bilangan: '+user_id
        );
    }
