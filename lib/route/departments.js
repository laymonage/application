...
function get_and_validate_department(args) {
  ...
  if (!validator.isFloat(allowance)) {
    req.session.flash_error(
      'Jatah cuti untuk '+department_name+' harus numerik'
    );
  } else if (!((0 <= allowance) && (allowance <= 50))) {
    req.session.flash_error(
      'Jatah cuti baru untuk '+department_name+' harus berada di antara 0.5 sampai 50 hari'
    );
  }
  ...
  if (!validator.isNumeric( boss_id ) ) {
    req.session.flash_error(
      'Referensi bos baru untuk '+department_name+' harus numerik'
    );
  } else if ( ! _.contains(
    ...
    req.session.flash_error(
      'Bos baru untuk '+department_name+' tidak diketahui'
    );
  }
...
router.post('/departments/', function(req, res){
  ...
  .then(() => {
    if ( ! req.session.flash_has_errors() ) {
      req.session.flash_message('Perubahan pada departemen telah disimpan');
    }
  ...
  .catch(error => {
    ...
    req.session.flash_error(
      'Gagal menambahkan departemen baru, silakan hubungi layanan pelanggan'
    );
...
router.post('/departments/delete/:department_id/', function(req, res){
  ...
  if (!validator.isInt(department_id)) {
    ...
    req.session.flash_error('Gagal menghapus departemen: parameter salah');
  ...
  .then(function(departments){
    ...
    if (! department_to_remove) {
      ...
      throw new Error(
        'Pengguna '+req.user.id+' mencoba menghapus ID departemen '+department_id
        +' yang tidak ada'
      );
    }
    ...
    if (department_to_remove.users.length > 0){
      req.session.flash_error(
        'Gagal menghapus departemen '+department_to_remove.name
          +' karena masih memiliki '
          +department_to_remove.users.length+' karyawan.'
      );

      throw new Error('Departemen masih memiliki karyawan');
    }
  ...
  .then(function(){
    req.session.flash_message('Departemen berhasil dihapus');
    return res.redirect_with_session('/settings/departments/');
  })
...
function promise_to_extract_company_and_department(req) {
  ...
  return Promise.try(function(){
    ...
    if ( ! validator.isInt(department_id)) {
      throw new Error('Pengguna '+req.user.id+' mencoba membuka departemen dengan ID'
      +department_id+' yang bukan bilangan bulat ');
    }
  ...
  .then(function(c){
    ...
    if ( ! company ) {
      throw new Error('Tidak dapat menentukan kantor!');
    }
  ...
  .then(function(departments){
    ...
    if ( ! department ) {
      throw new Error('ID departemen yang diberikan tidak ada');
    }
...
router.get('/departments/edit/:department_id/', function(req, res){
  ...
  .catch(function(error){
    ...
    req.session.flash_error(
      'Gagal mendapat detail untuk departemen yang diberikan'
    );
...
router.post('/departments/edit/:department_id/', function(req, res){
  ...
  .then(function(){
    if (req.body.remove_supervisor_id) {
      ...
      .then(function(){
        req.session.flash_message('Pengawas dihapus dari ' + department.name);
        return Promise.resolve(1);
      });
      ...
      .then(function(){
        req.session.flash_message('Pengawas ditambahkan ke ' + department.name);
        return Promise.resolve(1);
      });
    }
    ...
    .then(function(){
      req.session.flash_message('Departemen ' + department.name + ' diperbarui');
      return Promise.resolve(1);
    });
  })
  ...
  .catch(function(error){
    ...
    req.session.flash_error(
      "Gagal memperbarui detail departemen"
    );
...
function promise_to_update_department(args) {
  ...
  if ( req.session.flash_has_errors() ) {
    throw new Error("Parameter yang tidak valid diisi ketika mencoba memperbarui detail departemen");
  }
